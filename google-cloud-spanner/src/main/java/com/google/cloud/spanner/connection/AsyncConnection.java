 ...
 *
 * <p>An asynchronous connection to a Cloud Spanner database. Connections are not designed to be thread-safe.
 *
 * <p>Connections accept a number of additional SQL statements for setting or changing the state of
 * a {@link AsyncConnection}. These statements can only be executed using the {@link
 * AsyncConnection#execute(Statement)} method:
 *
 * <ul>
 ...
 *
 * <p>If {@link AsyncConnection#isRetryAbortsInternally()} is <code>true</code>, then the connection will
 ...
 * AsyncConnection#isRetryAbortsInternally()} to <code>false</code>.
 *
 * <p>Use {@link ConnectionOptions} to create a {@link AsyncConnection}.
 */
@InternalApi
public interface AsyncConnection extends AutoCloseable {
  /** Closes this connection. This is a no-op if the {@link AsyncConnection} has alread been closed. */
  @Override
  void close();
  ...
  /**
   * Sets autocommit on/off for this {@link AsyncConnection}. Connections in autocommit mode will apply
   ...
   * back.
   *
   * <p>A {@link AsyncConnection} that is in autocommit and read/write mode will allow all types of
   * statements: Queries, DML, DDL, and Mutations (writes). If the connection is in read-only mode,
   * only queries will be allowed.
   *
   * <p>{@link AsyncConnection}s in autocommit mode may also accept partitioned DML statements. See
   * {@link AsyncConnection#setAutocommitDmlMode(AutocommitDmlMode)} for more information.
   *
   ...
   *           {@link AsyncConnection#isRetryAbortsInternally()} is <code>true</code>, each read/write
   ...
   * default is <code>true</code>. When internal retry is enabled, the {@link AsyncConnection} will keep
  ...
  /**
   * Add a {@link TransactionRetryListener} to this {@link AsyncConnection} for testing and logging
  ...
  /**
   * Removes one existing {@link TransactionRetryListener} from this {@link AsyncConnection}, if it is
   ...
   *       methods {@link AsyncConnection#getReadTimestamp()} or {@link AsyncConnection#getCommitTimestamp()}
   *       will cause an exception.
   ...
   *     started). This method will only return false when the {@link AsyncConnection} is in autocommit
   *     mode and no explicit transaction has been started by calling {@link
   *     AsyncConnection#beginTransaction()}. If the {@link AsyncConnection} is not in autocommit mode, there
   ...
   */
  AsyncResultSet analyzeQueryAsync(Statement query, QueryAnalyzeMode queryMode);
   ...
   * AsyncConnection#executeUpdate(Statement)} in a loop. This method returns an array of long integers,
  ...
  /**
   * Buffers the given mutation locally on the current transaction of this {@link AsyncConnection}. The
   * mutation will be written to the database at the next call to {@link AsyncConnection#commit()}. The
   * value will not be readable on this {@link AsyncConnection} before the transaction is committed.
   *
   * <p>Calling this method is only allowed when not in autocommit mode. See {@link
   * AsyncConnection#write(Mutation)} for writing mutations in autocommit mode.
   *
   * @param mutation the {@link Mutation} to buffer for writing to the database on the next commit
   * @throws SpannerException if the {@link AsyncConnection} is in autocommit mode
   */
  void bufferedWrite(Mutation mutation);

  /**
   * Buffers the given mutations locally on the current transaction of this {@link AsyncConnection}. The
   * mutations will be written to the database at the next call to {@link AsyncConnection#commit()}. The
   * values will not be readable on this {@link AsyncConnection} before the transaction is committed.
   *
   * <p>Calling this method is only allowed when not in autocommit mode. See {@link
   * AsyncConnection#write(Iterable)} for writing mutations in autocommit mode.
   *
