...
public class SimpleParser {
    ...
    static String quotedStringValue(String quotedString, char quoteChar) {
      if (quotedString.length() < 2
        ...
        throw SpannerExceptionFactory.newSpannerException(
            ErrorCode.INVALID_ARGUMENT, quotedString + " is not a valid string");
      }
  ...
  List<TableOrIndexName> readColumnListInParentheses(String name) {
    if (eatToken("(")) {
      ...
      throw SpannerExceptionFactory.newSpannerException(
          ErrorCode.INVALID_ARGUMENT, String.format("missing opening parentheses for %s", name));
    }
  }
  ...
  List<TableOrIndexName> expressionListToColumnNames(String name, List<String> expressions) {
    if (expressions == null || expressions.isEmpty()) {
      throw SpannerExceptionFactory.newSpannerException(
          ErrorCode.INVALID_ARGUMENT, String.format("empty %s columns list", name));
    }
    ...
    for (String expression : expressions) {
      ...
      if (column == null) {
        throw SpannerExceptionFactory.newSpannerException(
            ErrorCode.INVALID_ARGUMENT, "Invalid column name: " + expression);
      }
      result.add(column);
    }
    return result;
  ...
  TypeDefinition readType() {
    ...
    if (eatToken("(")) {
      ...
      if (!eatToken(")")) {
        throw SpannerExceptionFactory.newSpannerException(
            ErrorCode.INVALID_ARGUMENT, "missing ')' for type length");
      }
    }
    if (eatToken("[")) {
      if (!eatToken("]")) {
        throw SpannerExceptionFactory.newSpannerException(
            ErrorCode.INVALID_ARGUMENT, "missing ']' for array definition");
      }
      array = true;
    }
  ...
  int parseInt(String value) {
    try {
      ...
      throw SpannerExceptionFactory.newSpannerException(
          ErrorCode.INVALID_ARGUMENT, "Invalid int value: " + value, exception);
    }
  }
  ...
  QuotedString readQuotedString(char quote) {
    ...
    if (pos >= sql.length()) {
      throw SpannerExceptionFactory.newSpannerException(
          ErrorCode.INVALID_ARGUMENT, "Unexpected end of expression");
    }
    boolean escaped = eatToken("e");
    if (sql.charAt(pos) != quote) {
      throw SpannerExceptionFactory.newSpannerException(
          ErrorCode.INVALID_ARGUMENT, "Invalid quote character: " + sql.charAt(pos));
    }
    int startPos = pos;
    ...
    throw SpannerExceptionFactory.newSpannerException(
        ErrorCode.INVALID_ARGUMENT, "Missing end quote character");
  }
  ...
  boolean skipTableHintExpression() {
    // TODO: Implement
    return true;
  }
  ...
  void throwIfHasMoreTokens() {
    ...
    if (getPos() < getSql().length()) {
      throw SpannerExceptionFactory.newSpannerException(
          ErrorCode.INVALID_ARGUMENT,
          "Syntax error. Unexpected tokens: " + getSql().substring(getPos()));
    }
  }
