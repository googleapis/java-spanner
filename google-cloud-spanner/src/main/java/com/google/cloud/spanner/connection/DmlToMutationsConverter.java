...
class DmlToMutationsConverter {
  static class DmlConversionException extends Exception {
    DmlConversionException(String message) {
      super(message);
    }

    DmlConversionException(String message, Exception cause) {
      super(message, cause);
    }
  }

  static List<Mutation> convert(Statement statement) throws DmlConversionException {
    ...
    if (parser.eatKeyword("insert")) {
      ...
      throw new DmlConversionException("Statement cannot be converted to a mutation: " + statement);
    }
  }

  static List<Mutation> convertInsert(Statement statement) throws DmlConversionException {
    SimpleParser parser = new SimpleParser(statement.getSql());
    if (!parser.eatKeyword("insert")) {
      throw new DmlConversionException("Statement is not an insert statement: " + statement);
    }
    parser.eatKeyword("into");
    TableOrIndexName table = parser.readTableOrIndexName();
    if (table == null) {
      throw new DmlConversionException("Invalid table name in insert statement: " + statement);
    }
    if (!parser.eatToken("(")) {
      throw new DmlConversionException(
          "Missing opening parentheses for columns list: " + statement);
    }
    ...
    if (!parser.eatKeyword("values")) {
      throw new DmlConversionException(
          "Missing 'values' keyword in insert statement: " + statement);
    }
    ...
    if (rows.isEmpty()) {
      throw new DmlConversionException("Missing values list in insert statement");
    }
  ...
  static List<Mutation> convertUpdate(Statement statement) throws DmlConversionException {
    SimpleParser parser = new SimpleParser(statement.getSql());
    if (!parser.eatKeyword("update")) {
      throw new DmlConversionException("Statement is not an update statement: " + statement);
    }
    TableOrIndexName table = parser.readTableOrIndexName();
    if (table == null) {
      throw new DmlConversionException("Invalid table name in update statement: " + statement);
    }
    ...
    if (mustHaveAlias || !parser.peekKeyword("set")) {
      ...
      if (alias == null) {
        throw new DmlConversionException("Invalid or missing table alias: " + statement);
      }
    }
    if (!parser.eatKeyword("set")) {
      throw new DmlConversionException("Missing keyword 'set' in update statement: " + statement);
    }
    ...
    if (assignmentsList == null || assignmentsList.isEmpty()) {
      throw new DmlConversionException("Empty assignment list in update statement: " + statement);
    }
    ...
    for (Entry<String, Value> entry :
      ...
      if (assignedColumns.contains(entry.getKey().toLowerCase())) {
        throw new DmlConversionException(
            "Column present in both WHERE clause and SET clause: " + statement);
      }
  ...
  static List<Mutation> convertDelete(Statement statement) throws DmlConversionException {
    SimpleParser parser = new SimpleParser(statement.getSql());
    if (!parser.eatKeyword("delete")) {
      throw new DmlConversionException("Statement is not a delete statement: " + statement);
    }
    parser.eatKeyword("from");
    TableOrIndexName table = parser.readTableOrIndexName();
    if (table == null) {
      throw new DmlConversionException("Invalid table name in delete statement: " + statement);
    }
    parser.skipTableHintExpression();
    String alias = table.getUnquotedName();
    boolean mustHaveAlias = parser.eatKeyword("as");
    ...
    List<Value> keyValues = keyValuesInOrder.build();
    if (keyValues.size() > 1) {
      // TODO: Allow this and check that only primary key columns have been used.
      throw new DmlConversionException(
          "DELETE statements with multiple WHERE clauses are currently not supported: "
              + statement);
    }
    Key.Builder keyBuilder = Key.newBuilder();
    for (Value value : keyValues) {
  ...
  static ImmutableMap<String, Value> convertAssignmentList(
    ...
    for (String assignment : assignments) {
      ...
      if (columnName.schema != null
        ...
        throw new DmlConversionException(
            "Invalid table or alias name in assignment: " + columnName);
      }
      if (!parser.eatToken("=")) {
  ...
  static DmlConversionException createInvalidAssignmentException(Statement statement) {
    return new DmlConversionException(
        "Invalid assignment expression. "
            + "Only assignments in the form 'column_name1 = <literal | parameter>[, column_name2 = <literal | parameter> [...]]' are supported: "
            + statement);
  }

  static DmlConversionException createInvalidWhereClauseException(Statement statement) {
    return new DmlConversionException(
  ...
  static Value convertExpressionToValue(Statement statement, String valueExpression)
    ...
    if (parameterName != null) {
      if (statement.getParameters().containsKey(parameterName)) {
        ...
        throw new DmlConversionException("Unknown parameter name: " + parameterName);
      }
    } else {
  ...
  static Value parseValue(String expression) throws DmlConversionException {
    ...
    if (parser.eatKeyword("null")) {
      ...
      try {
        ...
        throw new DmlConversionException(
            "Invalid or unsupported numeric literal: " + expression, e);
      }
      ...
      try {
        ...
        throw new DmlConversionException("Invalid or unsupported date literal: " + expression, e);
      }
      ...
      try {
        ...
        throw new DmlConversionException(
            "Invalid or unsupported timestamp literal: " + expression, e);
      }
