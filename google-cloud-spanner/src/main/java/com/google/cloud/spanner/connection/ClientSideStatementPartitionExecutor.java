...
import com.google.cloud.spanner.connection.ClientSideStatementValueConverters.PartitionCommandConverter;
...
class ClientSideStatementPartitionExecutor implements ClientSideStatementExecutor {
  ...
  String getParameterValue(ParsedStatement parsedStatement) {
    ...
    if (matcher.find() && matcher.groupCount() >= 1) {
      ...
      if (value != null) {
        String res = PartitionCommandConverter.INSTANCE.convert(value.trim());
        if (res != null) {
          return res;
        }
        throw SpannerExceptionFactory.newSpannerException(
            ErrorCode.INVALID_ARGUMENT, String.format("Invalid argument for PARTITION: %s", value));
      }
    }
    return null;
  }
