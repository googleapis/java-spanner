...
public abstract class AbstractStatementParser {
   ...
   * MethodDescriptor)} and {@link ReadWriteTransaction#runWithRetry(Callable)} to allow statements
   * to be cancelled, to timeout and to be retried. These methods require a {@link ParsedStatement}
   * as input. When the {@link Connection#commit()} method is called directly, we do not have a
   * {@link ParsedStatement}, and the method uses this statement instead in order to use the same
   * logic as the other statements.
   */
   ...
   * MethodDescriptor)}} and {@link ReadWriteTransaction#runWithRetry(Callable)} to allow statements
   * to be cancelled, to timeout and to be retried. These methods require a {@link ParsedStatement}
   * as input. When the {@link Connection#executeBatchUpdate(Iterable)} method is called, we do not
   * have one {@link ParsedStatement}, and the method uses this statement instead in order to use
  ...
  /** Returns the named parameter prefix that should be used for this dialect. */
  abstract String getNamedParameterPrefix();
  ...
  public ParametersInfo convertPositionalParametersToNamedParameters(char paramChar, String sql) {
    Preconditions.checkNotNull(sql);
    final String namedParamPrefix = getNamedParameterPrefix();
    ...
    while (index < sql.length()) {
      ...
      if (c == paramChar) {
        ...
        index = skipCommentsAndLiterals(sql, index, named);
      }
    }
  ...
  /**
   * Returns true if the given character is valid as the first character of an unquoted identifier.
   */
  boolean isValidIdentifierFirstChar(char c) {
    return Character.isLetter(c) || c == UNDERSCORE;
  }

  /** Returns true if the given character is valid in an unquoted identifier. */
  boolean isValidIdentifierChar(char c) {
    return isValidIdentifierFirstChar(c) || Character.isDigit(c) || c == DOLLAR;
  }

  int skipCommentsAndLiterals(String sql, int currentIndex, @Nullable StringBuilder result) {
  ...
  int skipQuoted(String sql, int startIndex, char startQuote, @Nullable StringBuilder result) {
  ...
  int skipSingleLineComment(String sql, int currentIndex, @Nullable StringBuilder result) {
  ...
  /** Skips to the end of the given multi-line comment. */
  ...
  /**
   * Appends a character to the given {@link StringBuilder}, or does nothing if the {@link
   * StringBuilder} is null.
   */
  static void appendIfNotNull(@Nullable StringBuilder result, char currentChar) {
    if (result != null) {
  ...
  /**
   * Appends a string to the given {@link StringBuilder}, or does nothing if the {@link
   * StringBuilder} is null.
   */
  static void appendIfNotNull(@Nullable StringBuilder result, String suffix) {
    if (result != null) {
  ...
  /**
   * Appends a tag to the given {@link StringBuilder}, or does nothing if the {@link StringBuilder}
   * is null.
   */
  static void appendIfNotNull(
