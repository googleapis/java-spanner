...
public class DatabaseAdminClientImplTest {
  ...
  @Test
  public void updataDatabase() throws Exception {
    com.google.cloud.spanner.Database database = client.newDatabaseBuilder(DatabaseId.of(DB_NAME))
        .setEnableDropProtection(true).build();
    Database database_proto = Database.newBuilder()
        .setName(DB_NAME)
    ...
    OperationFuture<Database, UpdateDatabaseMetadata> rawOperationFuture =
        OperationFutureUtil.immediateOperationFuture("updateDatabase", database_proto,
            UpdateDatabaseMetadata.getDefaultInstance());
    when(rpc.updateDatabase(database_proto,
        DatabaseField.toFieldMask(DatabaseField.ENABLE_DROP_PROTECTION))).thenReturn(
        rawOperationFuture);
    ...
    assertTrue(op.get().getEnableDropProtection());
  }
